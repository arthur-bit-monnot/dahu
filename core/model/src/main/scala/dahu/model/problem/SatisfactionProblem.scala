...
//      ctx => new DistributeImplication(ctx),
  ...
  def encode[X <: Int](root: X,
    ...
    val lt = OpenASG.build(coalgebra)(compiler).fixID
  ...
  def encodePresence[K](root: K,
    ...
    val _lt = OpenASG.build[K, Total, OptConst, cats.Id](coalgebra)(compilerPresence)
    val lt = _lt.fixID
  ...
  object OptConst {
    ...
    implicit object TreeNodeInstance extends TreeNode[OptConst] {
      ...
      override def foreachChild[K](n: OptConst[K])(f: K => Unit): Unit = {
        n.value.foreachChild(f)
        n.presence.foreach(f)
      }
    }
  }
