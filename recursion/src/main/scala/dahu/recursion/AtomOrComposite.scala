...
object AtomOrComposite {
  ...
  def cataM[M[_]: Monad, F[_]: Traverse, A](alg: FAlgebraM[M, F, AtomOrComposite[A]])(
      f: Fix[F]): M[A] =
  ...
  object stringBuilder {
    ...
    def composite(before: StringBuilder => Unit,
                  content: StringBuilder => Unit,
                  after: StringBuilder => Unit): Composite[StringBuilder => Unit] =
      Composite(content,
                c =>
                  sb => {
                    before(sb)
