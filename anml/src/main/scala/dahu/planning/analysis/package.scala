...
package object analysis {
  ...
  def abstractionHierarchy(model: CoreModel,
                           typ: AbstractionHierarchyType): Map[FluentTemplate, Int] = {
    val graph: Map[FluentTemplate, mutable.Set[FluentTemplate]] =
      model.collect {
        case FunctionDeclaration(x: FluentTemplate) => (x, mutable.Set[FluentTemplate]())
      }.toMap
    ...
    postProcessed.zipWithIndex
      .flatMap({ case (scc, lvl) => scc.toSeq.map((_, lvl)) })
      .toMap
  }

  def subHierarchy(fluents: Set[FluentTemplate],
                   actions: Seq[ActionTemplate]): Seq[Set[FluentTemplate]] = {
    val graph: Map[FluentTemplate, mutable.Set[FluentTemplate]] =
      fluents.map(_ -> mutable.Set[FluentTemplate]()).toMap

    def affectsFirstButNotSecond(action: ActionTemplate,
                                 f1: FluentTemplate,
                                 f2: FluentTemplate): Boolean = {
    ...
    for(f1 <- fluents; f2 <- fluents if f1 != f2) {
      ...
      if(!actions.exists(affectsFirstButNotSecond(_, f2, f1)) && !actions.exists(
           affectsFirstButNotSecond(_, f1, f2))) {
        graph(f1) += f2
        graph(f2) += f1
      }
