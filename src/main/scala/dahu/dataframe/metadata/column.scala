...
object ColumMetadata {
  ...
  implicit def key[K, V, F[_], C](
      implicit ev: C <:< ColumMetadata[K, V, F]): Key.Aux[C, K] = new Key[C] {
    override type Out = K
  }

  implicit def value[K, V, F[_], C](
      implicit ev: C <:< ColumMetadata[K, V, F]): Value.Aux[C, V] =
